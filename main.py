...
@app.post("/api/seed-workers")
def seed_workers(db: Session = Depends(get_db)):
    """Populates initial verified workers into the database if empty."""
    if db.query(models.Worker).count() > 0:
        return {"message": "Workers already seeded"}
    
    initial_workers = [
        models.Worker(name="Worker A", service="Plumbing", rating=4.8, lat=22.5726, lon=88.3639, exp_years=6, available=True),
        models.Worker(name="Worker B", service="Plumbing", rating=4.5, lat=22.5800, lon=88.3700, exp_years=3, available=True),
        models.Worker(name="Worker C", service="Electrical", rating=4.9, lat=22.5650, lon=88.3600, exp_years=8, available=True)
    ]
    db.add_all(initial_workers)
    db.commit()
    return {"message": "Seeded 3 initial workers successfully"}
